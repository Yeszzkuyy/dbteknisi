<x-app-layout>
    ...
    <form action="{{ route('leads.update', $lead) }}" method="POST" enctype="multipart/form-data"
        ...
        {{-- Data Customer --}}
        <section x-data="{ mode: '{{ old('customer_mode', 'existing') }}' }" class="pt-6 border-t border-slate-200">
        ...
        <section class="pt-6 border-t border-slate-200 space-y-4">
            ...
            <div>
                <label class="flex items-center gap-1.5 text-sm font-medium text-slate-700 mb-1">
                    Tambah Lampiran (BOQ / Kebutuhan User)
                    <x-info-tip tip="Unggah file BOQ awal, spesifikasi, atau dokumen kebutuhan dari user. Maksimal 5 file per simpan, 10 MB per file." />
                </label>
