            ...
            <form action="{{ route('admin.pos.update', $purchaseOrder) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="customer_id" required class="w-full rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500">
                        ...
                        @foreach($customers as $c) <option value="{{ $c->id }}" {{ old('customer_id', $purchaseOrder->customer_id) == $c->id ? 'selected' : '' }}>{{ $c->name }}</option> @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="date" name="issue_date" value="{{ old('issue_date', $purchaseOrder->issue_date->format('Y-m-d')) }}" required class="w-full rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500">
                </div>
                <div>
