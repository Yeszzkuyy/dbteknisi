<x-app-layout>
    <div class="max-w-3xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <div><h1 class="text-3xl font-bold text-slate-800">Buat Purchase Order</h1><p class="text-slate-500 mt-1">Buat PO baru untuk customer.</p></div>
            <a href="{{ route('admin.pos.index') }}" class="px-4 py-2.5 rounded-xl bg-slate-200 hover:bg-slate-300 text-slate-700 text-sm font-medium transition">Kembali</a>
        </div>
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <form action="{{ route('admin.pos.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="items" rows="4" required placeholder="Deskripsi barang/jasa yang dipesan..." class="w-full rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500">{{ old('items') }}</textarea>
