<x-app-layout>
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-3xl font-bold text-slate-800 dark:text-slate-100">{{ __('Buat Request Harga') }}</h1>
                <p class="text-slate-500 dark:text-slate-400 mt-1">{{ __('Ajukan daftar perangkat beserta spesifikasinya') }}</p>
            </div>
        ...
        <form action="{{ route('teknisi.request-hargas.store') }}" method="POST"
            ...
            <div>
                ...
                <textarea id="notes" name="notes" rows="3"
                          ...
                          class="w-full px-4 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-transparent bg-white dark:bg-slate-700 dark:border-slate-600 dark:text-slate-100">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
