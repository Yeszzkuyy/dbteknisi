<x-app-layout>
    ...
    <div class="max-w-[1400px] mx-auto space-y-6">
        ...
        {{-- Progress Pekerjaan --}}
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7 dark:border-slate-700 dark:bg-slate-800" data-reveal data-reveal-delay="2" aria-labelledby="progress-heading">
                    ...
                    @forelse($statusCounts as $status)
                        <li class="flex items-center justify-between gap-4 text-sm">
                            <span class="flex min-w-0 items-center gap-2.5">
                                ...
                                <span class="truncate font-semibold text-slate-700">{{ $status['name'] }}</span>
                            </span>
                            <span class="shrink-0 font-bold text-slate-800 tabular-nums">{{ $status['count'] }} Project</span>
                        </li>
                    @empty
        ...
        {{-- Project Terbaru --}}
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7 dark:border-slate-700 dark:bg-slate-800" data-reveal data-reveal-delay="3" aria-labelledby="recent-projects-heading">
            <div class="flex flex-wrap items-end justify-between gap-3 border-b border-slate-100 pb-4 dark:border-slate-700">
                <div>
            ...
            @forelse($recentProjects as $project)
                <article class="flex flex-col gap-2 border-b border-slate-100 py-3 last:border-0 sm:flex-row sm:items-center dark:border-slate-700">
                    <div class="min-w-0 flex-1">
                        <p class="truncate text-sm font-semibold text-slate-800">{{ $project->project_name }}</p>
                    ...
                    <div class="w-24 shrink-0 text-right text-xs text-slate-400">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2" data-reveal data-reveal-delay="4">
            {{-- Aktivitas Terbaru --}}
            <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7 dark:border-slate-700 dark:bg-slate-800" aria-labelledby="recent-activities-heading">
                ...
                @forelse($activities as $activity)
                    <article class="flex items-start gap-4 border-b border-slate-100 py-4 last:border-0 last:pb-0 dark:border-slate-700">
            ...
            {{-- Teknisi Aktif --}}
            <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7 dark:border-slate-700 dark:bg-slate-800" aria-labelledby="active-technicians-heading">
                <div class="border-b border-slate-100 pb-4 dark:border-slate-700">
                ...
                @forelse($activeTechnicians as $technician)
                    <article class="flex items-start gap-3 border-b border-slate-100 py-3 {{ $loop->last && $idleTechnicians->isEmpty() ? 'border-0' : '' }} dark:border-slate-700">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="truncate text-sm font-semibold text-slate-800">{{ $technician->name }}</p>
                ...
                @if($idleTechnicians->isNotEmpty() && $activeTechnicians->isNotEmpty())
                    <p class="pb-1 pt-4 text-xs font-semibold uppercase tracking-wide text-slate-400">Tidak Aktif</p>
                @endif

                @forelse($idleTechnicians as $technician)
                    <article class="flex items-center gap-3 border-b border-slate-100 py-3 opacity-60 last:border-0 dark:border-slate-700">
                        <x-user-avatar :user="$technician" size="w-10 h-10" text="text-sm" />
                        <div class="min-w-0 flex-1">
                            <p class="truncate text-sm font-semibold text-slate-800">{{ $technician->name }}</p>
