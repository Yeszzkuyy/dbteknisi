<x-app-layout>
    ...
    <div id="dash-skeleton" class="dash-scope" hidden>
    ...
    <div class="dash-scope mx-auto max-w-[1400px] space-y-6">
        {{-- Overview header --}}
        <section class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white px-5 py-6 shadow-sm sm:px-7 sm:py-7 dark:border-[#363d5c] dark:bg-[#222840]" data-reveal>
            ...
            <div class="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div class="min-w-0">
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <span class="hidden h-1 w-1 rounded-full bg-slate-300 sm:block dark:bg-[#545d7e]"></span>
                        <span class="text-xs font-medium text-slate-400">{{ __('Ringkasan operasional') }}</span>
                    </div>
                    <h1 class="mt-3 max-w-3xl text-2xl font-extrabold tracking-tight text-slate-900 sm:text-3xl dark:text-[#e2e8f8]">
                        {{ __('Selamat datang kembali,') }} {{ auth()->user()->name }}
                    </h1>
                    <p class="mt-2 max-w-2xl text-sm leading-relaxed text-slate-500 dark:text-[#8d96b8]">
                        {{ __('Pantau kondisi customer, project, dan dokumentasi 3DY Group dari satu tempat.') }}
                    </p>
                </div>

                <div class="flex shrink-0 items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-[#363d5c] dark:bg-[#2a3150]/60">
                    ...
                    <div>
                        ...
                        <p class="mt-0.5 text-sm font-bold text-slate-700 dark:text-[#e2e8f8]">Overview 3DY Group</p>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section aria-labelledby="dashboard-summary-heading" data-reveal data-reveal-delay="1">
            <div class="mb-3 flex flex-wrap items-end justify-between gap-2 px-1">
                <div>
                    <h2 id="dashboard-summary-heading" class="text-lg font-bold text-slate-900 dark:text-[#e2e8f8]">{{ __('Ringkasan utama') }}</h2>
                    <p class="mt-0.5 text-sm text-slate-500 dark:text-[#8d96b8]">{{ __('Angka terbaru dari data operasional yang tersedia.') }}</p>
                </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                @foreach($dashboardCards as $card)
                    <article class="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition duration-300 hover:border-accent-200 sm:p-6 dark:border-[#363d5c] dark:bg-[#222840] dark:hover:border-accent-500/40">
                        ...
                        <div class="relative flex items-start justify-between gap-4">
                            <div class="min-w-0">
                                ...
                                <p class="mt-3 text-3xl font-extrabold tracking-tight text-slate-900 tabular-nums dark:text-[#e2e8f8]"
                                   x-data="counter({{ $card['value'] }})" x-init="start()" x-text="display">{{ $card['value'] }}</p>
                                <p class="mt-1 text-xs font-medium text-slate-500 dark:text-[#8d96b8]">{{ $card['description'] }}</p>
                            </div>
        ...
        {{-- Recent activities --}}
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7 dark:border-[#363d5c] dark:bg-[#222840]" data-reveal data-reveal-delay="2" aria-labelledby="recent-activities-heading">
            <div class="flex flex-wrap items-end justify-between gap-3 border-b border-slate-100 pb-4 dark:border-[#363d5c]">
                <div>
                    <h2 id="recent-activities-heading" class="text-lg font-bold text-slate-900 dark:text-[#e2e8f8]">{{ __('Aktivitas terbaru') }}</h2>
                    <p class="mt-0.5 text-sm text-slate-500 dark:text-[#8d96b8]">{{ __('Perubahan terakhir yang tercatat pada project.') }}</p>
                </div>
                <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500 dark:bg-[#363d5c] dark:text-[#8d96b8]">
                    {{ $activities->count() }} {{ __('aktivitas') }}
                </span>
            </div>

            @forelse($activities as $activity)
                <article class="group flex items-start gap-4 border-b border-slate-100 py-4 last:border-0 last:pb-0 dark:border-[#363d5c]">
                    <div class="shrink-0">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between sm:gap-4">
                            <p class="min-w-0 text-sm leading-snug text-slate-700 dark:text-[#e2e8f8]">
                                <span class="font-bold text-slate-900 dark:text-[#e2e8f8]">{{ $activity->user?->name ?? 'System' }}</span>
                                <span class="text-slate-600 dark:text-[#8d96b8]">{{ $activity->title ?? __('Aktivitas') }}</span>
                            </p>
                        ...
                        <p class="mt-1 truncate text-xs text-slate-400 dark:text-[#545d7e]">
            ...
            @empty
                <div class="rounded-xl bg-slate-50 px-5 py-12 text-center dark:bg-[#2a3150]/60">
                    <span class="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-white text-slate-300 shadow-sm dark:bg-[#363d5c] dark:text-[#545d7e]">
                        <x-icon name="activity" class="h-6 w-6" />
                    </span>
                    <p class="mt-4 text-sm font-medium text-slate-500 dark:text-[#8d96b8]">{{ __('Belum ada aktivitas yang tercatat.') }}</p>
                    <p class="mt-1 text-xs text-slate-400 dark:text-[#545d7e]">{{ __('Aktivitas project akan muncul di sini.') }}</p>
                </div>
            @endforelse
        </section>
    </div>
